	...
	refresh: function(frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(__("Send Emails to Suppliers"), function() {
				frappe.call({
					method: 'shopper.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails',
					freeze: true,
					args: {
						...
						(data) => {
							var w = window.open(
								frappe.urllib.get_full_url(
									"/api/method/shopper.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
									new URLSearchParams({
										name: frm.doc.name,
	...
	make_supplier_quotation: function(frm) {
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return frappe.call({
					type: "GET",
					method: "shopper.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
shopper.buying.RequestforQuotationController = class RequestforQuotationController extends shopper.buying.BuyingController {
	refresh() {
		var me = this;
		super.refresh();
		if (this.frm.doc.docstatus===0) {
			...
			this.frm.add_custom_button(__('Possible Supplier'), function() {
				...
				var dialog = new frappe.ui.Dialog({
					...
					primary_action: (args) => {
						...
						shopper.utils.map_current_doc({
							method: "shopper.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(__('Link to Material Requests'),
				function() {
					shopper.buying.link_to_mrs(me.frm);
				}, __("Tools"));
			...
			fields: [
				{
					...
					onchange() {
						if(dialog.get_value('search_type') == 'Tag'){
							frappe.call({
								method: 'shopper.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag',
							}).then(r => {
...
extend_cscript(cur_frm.cscript, new shopper.buying.RequestforQuotationController({frm: cur_frm}));
