...
shopper.buying = {
	setup_buying_controller: function() {
		shopper.buying.BuyingController = class BuyingController extends shopper.TransactionController {
			setup() {
				super.setup();
				...
				if(this.frm.get_field('shipping_address')) {
					this.frm.set_query("shipping_address", function() {
						if(me.frm.doc.customer) {
							...
							return shopper.queries.company_address_query(me.frm.doc)
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query('supplier', shopper.queries.supplier);
				me.frm.set_query('contact_person', shopper.queries.contact_query);
				me.frm.set_query('supplier_address', shopper.queries.address_query);

				me.frm.set_query('billing_address', shopper.queries.company_address_query);
				shopper.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);

				if(this.frm.fields_dict.supplier) {
					this.frm.set_query("supplier", function() {
						return{	query: "shopper.controllers.queries.supplier_query" }});
				}
				...
				this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
					...
					return {
						query: "shopper.controllers.queries.item_manufacturer_query",
			...
			supplier_address() {
				shopper.utils.get_address_display(this.frm);
				shopper.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
			}
			...
			warehouse(doc, cdt, cdn) {
				...
				if(item.item_code && item.warehouse) {
					return this.frm.call({
						method: "shopper.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(__("Product Bundle"), function() {
					shopper.buying.get_items_from_product_bundle(me.frm);
				}, __("Get Items From"));
			}

			shipping_address(){
				var me = this;
				shopper.utils.get_address_display(this.frm, "shipping_address",
					"shipping_address_display", true);
			}

			billing_address() {
				shopper.utils.get_address_display(this.frm, "billing_address",
			...
			manufacturer(doc, cdt, cdn) {
				...
				if(row.manufacturer) {
					frappe.call({
						method: "shopper.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
...
shopper.buying.link_to_mrs = function(frm) {
	frappe.call({
...
shopper.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "shopper.stock.get_item_details.get_default_bom",
				args: {
...
shopper.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "shopper.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
