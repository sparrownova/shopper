frappe.provide("shopper.financial_statements");
...
shopper.financial_statements = {
	...
	"formatter": function(value, row, column, data, default_formatter) {
		if (data && column.fieldname=="account") {
			...
			if (data.account) {
				column.link_onclick =
					"shopper.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function(report) {
		...
		shopper.financial_statements.filters = get_filters()

		let fiscal_year = shopper.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		},
		{
			...
			"fieldtype": "Link",
			"options": "Fiscal Year",
			"default": shopper.utils.get_fiscal_year(frappe.datetime.get_today()),
			"reqd": 1,
		...
		// accordingly.
		{
			...
			"label": __("Currency"),
			"fieldtype": "Select",
			"options": shopper.get_presentation_currency_list()
		},
		{
