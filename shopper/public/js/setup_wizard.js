...
frappe.setup.on("before_load", function () {
	shopper.setup.slides_settings.map(frappe.setup.add_slide);
});

shopper.setup.slides_settings = [
	{
		// Organization
		name: 'organization',
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = shopper.setup.fiscal_years[country];
		...
		load_chart_of_accounts: function (slide) {
			...
			if (country) {
				frappe.call({
					method: "shopper.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: 'shopper.accounts.utils.get_coa',
				args: {
