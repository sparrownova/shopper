	...
	setup: function(frm) {
		...
		frm.make_methods = {
			'Quotation': () => frappe.model.open_mapped_doc({
				method: "shopper.selling.doctype.customer.customer.make_quotation",
				frm: cur_frm
			}),
			'Opportunity': () => frappe.model.open_mapped_doc({
				method: "shopper.selling.doctype.customer.customer.make_opportunity",
				frm: cur_frm
			})
		}
		...
		frm.set_query('customer_primary_contact', function(doc) {
			return {
				query: "shopper.selling.doctype.customer.customer.get_customer_primary_contact",
				filters: {
					'customer': doc.name
				}
			}
	...
	refresh: function(frm) {
		...
		if(!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Pricing Rule'), function () {
				shopper.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name);
			}, __('Create'));
			...
			// indicator
			shopper.utils.set_party_dashboard_indicators(frm);
	...
	show_party_link_dialog: function(frm) {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function({ supplier }) {
				frappe.call({
					method: 'shopper.accounts.doctype.party_link.party_link.create_party_link',
					args: {
