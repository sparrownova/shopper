...
import shopper
from shopper.setup.default_energy_point_rules import get_default_energy_point_rules
from shopper.setup.doctype.incoterm.incoterm import create_incoterms

from .default_success_action import get_default_success_action

default_mail_footer = """<div style="padding: 7px; text-align: right; color: #888"><small>Sent via
	<a style="color: #888" href="http://shopper.org">Shopper</a></div>"""
...
def check_setup_wizard_not_completed():
	if cint(frappe.db.get_single_value("System Settings", "setup_complete") or 0):
		message = """Shopper can only be installed on a fresh site where the setup wizard is not completed.
...
def check_frappe_version():
	...
	shopper_version = major_version(shopper.__version__)

	if frappe_version == shopper_version:
		return

	click.secho(
		f"You're attempting to install Shopper version {shopper_version} with Frappe version {frappe_version}. "
...
def add_standard_navbar_items():
	...
	for item in shopper_navbar_items:
...
def add_app_name():
	frappe.db.set_single_value("System Settings", "app_name", "Shopper")
