...
frappe.ui.form.on("Item", {
	...
	onload: function(frm) {
		shopper.item.setup_queries(frm);
	...
	refresh: function(frm) {
		...
		if (frm.doc.has_variants) {
			...
			if(frm.doc.variant_based_on==="Item Attribute") {
				frm.add_custom_button(__("Single Variant"), function() {
					shopper.item.show_single_variant_dialog(frm);
				}, __('Create'));
				frm.add_custom_button(__("Multiple Variants"), function() {
					shopper.item.show_multiple_variants_dialog(frm);
				}, __('Create'));
			} else {
				frm.add_custom_button(__("Variant"), function() {
					shopper.item.show_modal_for_manufacturers(frm);
				}, __('Create'));
			}
		...
		shopper.item.edit_prices_button(frm);
		shopper.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			shopper.item.make_dashboard(frm);
		}
	...
	validate: function(frm){
		shopper.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function(frm) {
		...
		frappe.call({
			method: "shopper.stock.doctype.item.item.get_asset_naming_series",
			callback: function(r) {
...
$.extend(shopper.item, {
	setup_queries: function(frm) {
		...
		frm.fields_dict.customer_items.grid.get_field("customer_name").get_query = function(doc, cdt, cdn) {
			return { query: "shopper.controllers.queries.customer_query" }
		}
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require('item-dashboard.bundle.js', function() {
				...
				shopper.item.item_dashboard = new shopper.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: 'shopper.stock.dashboard.item_dashboard.get_data',
					template: 'item_dashboard_list'
				});
				shopper.item.item_dashboard.refresh();
			});
		}
	},
	...
	show_modal_for_manufacturers: function(frm) {
		...
		dialog.set_primary_action(__('Create'), function() {
			...
			frappe.call({
				method: "shopper.controllers.item_variant.get_variant",
				args: data,
				callback: function(r) {
	...
	show_multiple_variants_dialog: function(frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__('Create Variants'), () => {
				...
				frappe.call({
					method: "shopper.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						"item": frm.doc.name,
			...
			frappe.call({
				...
				callback: function(r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "shopper.controllers.item_variant.create_variant",
							args: {
								"item": frm.doc.name,
								"args": d.get_values()
							},
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on('input', function(e) {
					...
					frappe.call({
						method: "shopper.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function(frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "shopper.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					"uom": row.uom,
...
frappe.tour['Item'] = [
	...
	},
	{
		...
		description: __("If you are maintaining stock of this Item in your Inventory, Shopper will make a stock ledger entry for each transaction of this item.")
	},
	{
