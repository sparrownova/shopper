...
from shopper.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": shopper.get_default_currency(),
	...
	def recalculate_bin_qty(self, new_name):
		from shopper.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"shopper.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def get_asset_naming_series():
	from shopper.assets.doctype.asset.asset import get_asset_naming_series
