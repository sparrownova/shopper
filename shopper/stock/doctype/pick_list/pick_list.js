...
frappe.ui.form.on('Pick List', {
	setup: (frm) => {
		...
		frm.set_query('work_order', () => {
			return {
				query: 'shopper.stock.doctype.pick_list.pick_list.get_pending_work_orders',
				filters: {
					'company': frm.doc.company
				}
			};
	...
	refresh: (frm) => {
		...
		if (frm.doc.docstatus === 1) {
			frappe.xcall('shopper.stock.doctype.pick_list.pick_list.target_document_exists', {
		...
		frappe.db.get_value('Work Order',
			...
			frappe.prompt({
				...
				shopper.utils.map_current_doc({
					method: 'shopper.manufacturing.doctype.work_order.work_order.create_pick_list',
					target: frm,
	...
	create_delivery_note: (frm) => {
		frappe.model.open_mapped_doc({
			method: 'shopper.stock.doctype.pick_list.pick_list.create_delivery_note',
			frm: frm
		});

	},
	create_stock_entry: (frm) => {
		frappe.xcall('shopper.stock.doctype.pick_list.pick_list.create_stock_entry', {
			'pick_list': frm.doc,
		}).then(stock_entry => {
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new shopper.utils.BarcodeScanner(opts);
...
function get_item_details(item_code, uom=null) {
	if (item_code) {
		return frappe.xcall('shopper.stock.doctype.pick_list.pick_list.get_item_details', {
			item_code,
			uom
		});
	}
