...
frappe.ui.form.on("Stock Reconciliation", {
	...
	scan_barcode: function(frm) {
		const barcode_scanner = new shopper.utils.BarcodeScanner({frm:frm});
	...
	get_items: function(frm) {
		...
		frappe.prompt(fields, function(data) {
			frappe.call({
				method: "shopper.stock.doctype.stock_reconciliation.stock_reconciliation.get_items",
				args: {
	...
	set_valuation_rate_and_qty: function(frm, cdt, cdn) {
		...
		if(d.item_code && d.warehouse) {
			frappe.call({
				method: "shopper.stock.doctype.stock_reconciliation.stock_reconciliation.get_stock_balance_for",
				args: {
	...
	toggle_display_account_head: function(frm) {
		frm.toggle_display(['expense_account', 'cost_center'],
			shopper.is_perpetual_inventory_enabled(frm.doc.company));
	},
	purpose: function(frm) {
		frm.trigger("set_expense_account");
	},
	set_expense_account: function(frm) {
		if (frm.doc.company && shopper.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "shopper.stock.doctype.stock_reconciliation.stock_reconciliation.get_difference_account",
				args: {
...
frappe.ui.form.on("Stock Reconciliation Item", {
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		shopper.utils.pick_serial_and_batch_bundle(frm, cdt, cdn, "Inward");
	}

});

shopper.stock.StockReconciliation = class StockReconciliation extends shopper.stock.StockController {
	setup() {
		var me = this;
	...
	refresh() {
		if(this.frm.doc.docstatus > 0) {
			...
			if (shopper.is_perpetual_inventory_enabled(this.frm.doc.company)) {
...
cur_frm.cscript = new shopper.stock.StockReconciliation({frm: cur_frm});
